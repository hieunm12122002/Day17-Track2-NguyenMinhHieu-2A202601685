...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    """
    con.executemany(
        f"insert into {TABLE} values (?, ?, ?, ?, ?, ?, ?, ?) on conflict(event_id) do update set ticket_id=excluded.ticket_id, customer_id=excluded.customer_id, customer_name=excluded.customer_name, event_type=excluded.event_type, latency_ms=excluded.latency_ms, event_time=excluded.event_time, _ingested_at=excluded._ingested_at",
        [
            (
